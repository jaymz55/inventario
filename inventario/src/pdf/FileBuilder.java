...
public abstract class FileBuilder implements Serializable {
    ...
    //Este método genera el archivo
    
    public File getFile() {
        try {
            initTempFile(); //Genera el archivo temporal con el nombre y terminación
            resetContent(); //No hace nada
            buildFileContent(); //Genera el contenido (Header, columns, rows, etc.)
            writeToFile(); //No hace nada
        } catch (Exception e) {
            e.printStackTrace();
        }
        return file;
